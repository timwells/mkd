    ...
    <VaCard v-if="tabSelect === 'Positions'" class="tab-content rounded-xl" outlined>
      <VaCardContent>
        <EasyDataTable
          ...
          :body-row-class-name="getRowPLClass"
          alternating
...
const getRowPLClass = (item: any) => item.unrealizedProfitLoss < 0 ? 'red-text-row' : '';
...
.red-text-row {
  color: #ef4444 !important;          /* red text for whole row */
  /* Optional: also make background subtle */
  background-color: #fef2f2;
}
.red-text-row td {
  color: #dc2626 !important;     /* Tailwind red-600 or any color */
