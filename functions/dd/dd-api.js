      ...
      .each((_, el) => {
        ...
        if (cells.length) {
          ...
          dividendData.push({
            ...
            exDate: exDateStr || '',
            amount: +parseFloat(($(amountCell).text() || '').replace(/[^0-9.]/g, '')).toFixed(2) || 0.0,
...
export const dividenhistory = async (divlink) => {
  ...
  try {
    ...
    // Select all table rows except the header
    $('#ctl00_ContentPlaceHolder1_DividendHistoryGridView tr')
      .slice(1)
      .each((_, el) => {
        const cells = $(el).find('td')
        if (cells.length) {
          ...
          dividends.push({
            ...
            payDate: payDate || '',
            type: type || '',
            amount: +parseFloat((amountCell || '').replace(/[^0-9.]/g, '')).toFixed(2) || 0.0,
          })
        }
      })
  } catch (e) {
