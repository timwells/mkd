  ...
  $('.trades>tbody>tr').each((i, e) => {
    ...
    $(e)
      ...
      .each((i, e) => {
        ...
        if (textId.includes(i)) {
          rowObj[headers[i]] = item
        } else {
          rowObj[headers[i]] = item.length > 0 ? parseFloat(item) : item
        }
      })
...
export const trades = async () => {
  ...
  const [meta, cached] = await Promise.all([_readCacheMeta(NT_HASH_KEY_NAME), _readCacheData(NT_CONTENT_NAME)])
