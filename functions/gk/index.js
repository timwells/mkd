...
export const gk = onRequest({ secrets: [geminiApiKey] }, async (req, res) => {
  try {
    const prompt = req.body?.prompt ?? 'Give a detailed explanation of GenKit in clear steps.'
    ...
    const text =
      typeof result === 'string'
        ? result
        : result.text ?? result.output_text ?? result.content ?? JSON.stringify(result)
