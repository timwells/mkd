...
const seriesTA = (ohlcSeries, values) => {
  return [...[...new Array(ohlcSeries.length - values.length)].map((d, i) => ohlcSeries[i].close), ...values].map(
    (e, i) => ({ time: ohlcSeries[i].time, value: +e.toFixed(3) }),
  )
}

export const getDataImpl = async (exchange, symbol, precision, period) => {
  ...
  try {
    ...
    const vwapSeries = data.ChartBars.reduce((arr, e) => {
      if (e.VWAP !== null) {
        arr.push({ time: e.PricingDate, value: +e.VWAP.toFixed(3) })
      }
      return arr
    }, [])
    ...
    const volSeries = data.ChartBars.reduce((arr, e) => {
      if (e.Volume !== null) {
        if (e.Open != null && e.Close != null) {
          ...
          if (e.Close.Amount > e.Open.Amount) {
            direction = 1
          } // 'Up Day' - Green volume bar
          else if (e.Close.Amount < e.Open.Amount) {
            direction = -1
          } // 'Down Day' Red volume bar
          else {
            direction = 0
          } //  Gray or no specific color
...
export const getDataImpl2 = async (exchange, symbol, precision, period) => {
  const resource = `${API_HOST}/${API_HISTORICAL_PATH}/${exchange}:${symbol}?apikey=${API_KEY}&precision=${precision}&timeFrame=${period}`
  try {
    const { data } = await axios.get(resource, { headers: HEADERS })
...
export const getDataValuesImpl = async (exchange, symbol, precision, period) => {
  const resource = `${API_HOST}/${API_HISTORICAL_PATH}/${exchange}:${symbol}?apikey=${API_KEY}&precision=${precision}&timeFrame=${period}`
  try {
    const { data } = await axios.get(resource, { headers: HEADERS })
